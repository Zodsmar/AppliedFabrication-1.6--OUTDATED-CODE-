...
public class AppliedFabrication {
	...
	public static CreativeTabs AFTab = new AFTab(CreativeTabs.getNextID(),
			ModInfo.MODNAME);
	public static CreativeTabs AFComponents = new AFComponentsTab(
			CreativeTabs.getNextID(), "AFComponents");
	...
	public static void init(FMLInitializationEvent event) {
		LanguageRegistry.instance().addStringLocalization(
				"itemGroup." + ModInfo.MODNAME, "en_US", ModInfo.MODNAME);
		LanguageRegistry.instance().addStringLocalization(
				"itemGroup." + "AFComponents", "en_US", "AFComponents");
		...
		NetworkRegistry.INSTANCE
				.registerGuiHandler(ModInfo.MODNAME, guiHandler);
		GameRegistry.registerTileEntity(TileEntityFabTable.class,
				"Fabrication Table");

		ChestGenHooks.addItem(ChestGenHooks.VILLAGE_BLACKSMITH,
				new WeightedRandomChestContent(new ItemStack(Items.hammer), 1,
						1, 80));
		ChestGenHooks.addItem(ChestGenHooks.VILLAGE_BLACKSMITH,
				new WeightedRandomChestContent(new ItemStack(
						Items.mysticalHammer), 1, 1, 2));

		ChestGenHooks.addItem(ChestGenHooks.PYRAMID_DESERT_CHEST,
				new WeightedRandomChestContent(new ItemStack(
						Items.mysticalHammer), 1, 1, 3));
		ChestGenHooks.addItem(ChestGenHooks.STRONGHOLD_LIBRARY,
				new WeightedRandomChestContent(new ItemStack(
						Items.mysticalHammer), 1, 1, 10));
	...
	/**
	 * private static void RemoveRecipe(ItemStack resultItem) { ItemStack
	 * recipeResult = null; ArrayList<?> recipes = (ArrayList<?>)
	 * CraftingManager.getInstance() .getRecipeList(); for (int scan = 0; scan <
	 * recipes.size(); scan++) { IRecipe tmpRecipe = (IRecipe)
	 * recipes.get(scan); recipeResult = tmpRecipe.getRecipeOutput(); if
	 * (recipeResult != null) { if (recipeResult.itemID == resultItem.itemID &&
	 * recipeResult.getItemDamage() == resultItem .getItemDamage()) {
	 * System.out.println("Removed Recipe: " + recipes.get(scan) + " -> " +
	 * recipeResult); recipes.remove(scan); scan--; } } } }
	 **/
